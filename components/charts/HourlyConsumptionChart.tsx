...
export function HourlyConsumptionChart({ data, showPvpc, ariaLabel = 'Gráfico de consumo horario' }: Props) {
  ...
  return (
    ...
    <ResponsiveContainer width="100%" height={180}>
      <ComposedChart data={data} margin={{ top: 4, right: showPvpc ? 36 : 8, bottom: 0, left: -10 }}>
        <defs>
        ...
        {showPvpc && (
          <YAxis yAxisId="pvpc" orientation="right" tick={{ fontSize: 10, fill: '#a78bfa' }} tickLine={false} axisLine={false} tickFormatter={(v: number) => v.toFixed(2)} width={36} />
        )}
